...
public class AnalizadorLexico {
   ...
   //public static final String tablaTokensSalidaErrores = "./TablaErrores.txt";
    ...
    public void leerArchivo(String nombreArchivo) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(nombreArchivo))) {
            ...
            while ((linea = br.readLine()) != null) {
                ArrayList<Token> lt;  // tokens de la linea
                numLinea++;
                linea = linea.replaceAll("/\\.?\\*/", "").trim();

                Linea tmp = new Linea(linea, numLinea);
                lt = analizarLinea(tmp);

                classLexema = clasificacionToken(lt, numLinea);
            }
    ...
    // supondre que lo que quiere hacer brandon es que este metodo se ejecute con un arraylist
    // de tokens en lugar de con un string 
    // true -> token es error
    public boolean clasificacionToken(ArrayList<Token> lexemas, int numLinea) {
        boolean noClasificadoLexeme = false;
        //String[] tokens = analizarLexemas(lexema); comentado pq esto se usaba en la version vieja

        //for (String token : tokens) {
        for (int i = 0; i < lexemas.size(); i++) {
            String token = lexemas.get(i).getLexema();
    ...
    // analizarLexemas recibe un arraylist de tokens y retorna un arreglo de strings que
    // continue los strings del arraylist que si cumplieron con la regex; es mi suposicion
    public String[] analizarLexemas(ArrayList<Token> lexemas) {
        // La expresión regular para validar los tokens
        Pattern pattern = Pattern.compile(
            "\".*?\"|//.*?$|/\\*.*?\\*/|:=|&&|<=|>=|==|!=|\\|\\||\\||-?\\d+\\.\\d*|[-+*/;=,<>():!]|\\d+!|(\"[^\"]+\")|\\b[a-zA-Z\\d_]+\\b[#@%&$?]*|\\.[^ \\t\\n\\r\\f\\v]+|="
        );
    
        ArrayList<String> tokensValidos = new ArrayList<>();
    
        for (Token token : lexemas) {
            String lexema = token.getLexema();
            Matcher matcher = pattern.matcher(lexema);
        
            // Si el lexema completo coincide con el patrón, lo añadimos
            if (matcher.matches()) {
                tokensValidos.add(lexema);
            }
            // Si queremos buscar múltiples coincidencias dentro de un mismo lexema (opcional)
            
            while (matcher.find()) {
                tokensValidos.add(matcher.group());
            }
            
        }
    
        return tokensValidos.toArray(new String[0]);
    }
